...
fn render_header(frame: &mut Frame, app: &App, area: Rect) {
    let path = app.session.vm().directory.dir_path();
    ...
    let var_count = app.session.vm().directory.var_count();
    let subdir_count: usize = app.session.vm().directory.subdir_names().count();

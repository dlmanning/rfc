...
impl App {
    ...
    pub fn apply_edit(&mut self) -> Result<(), String> {
        ...
        let stack = self.session.vm().stack_contents();
        ...
        match self.session.eval_repl(&content) {
            Ok(()) => {
                ...
                let new_depth = self.session.vm().stack_contents().len();
    ...
    pub fn replace_selected(&mut self) -> Result<bool, String> {
        ...
        let original_depth = self.session.vm().stack_contents().len();
        if level > original_depth {
            return Err("Selected level no longer exists".to_string());
        }
    ...
    pub fn select_level(&mut self, level: usize) {
        ...
        // If it's a Bytes value starting with plot commands, set up plot view
        // (Plot detection is heuristic - plots start with known command bytes)
        if let Value::Bytes(data) = value {
            self.plot_view = Some(PlotViewState::new(data.to_vec()));
        } else {
            self.plot_view = None;
        }
    }
    ...
    pub fn select_up(&mut self) {
        let stack_len = self.session.vm().stack_contents().len();
        if stack_len == 0 {
            return;
        }
    ...
    pub fn should_show_preview(&self) -> bool {
        if let Some(level) = self.selected_level {
            ...
            if let Some(value) = stack.get(stack.len().saturating_sub(level)) {
                // Show preview for Bytes values (may be plot data)
    ...
    pub fn update_plot_preview(&mut self) {
        ...
        if let Some(level) = self.selected_level {
            let stack = self.session.vm().stack_contents();
            if let Some(Value::Bytes(data)) = stack.get(stack.len().saturating_sub(level)) {
                self.plot_view = Some(PlotViewState::new(data.to_vec()));
            }
        }
    }

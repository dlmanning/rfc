//! Virtual machine for executing WASM-aligned bytecode.
//!
...
use crate::{
    PROG_LIB,
    bytecode::{CatchKind, Opcode, read_f64, read_leb128_i64, read_leb128_u32, read_u16, read_u32},
    debug::{DebugEvent, DebugState},
    directory::Directory,
    executor::{ExecuteContext, ExecutorRegistry, RplException},
...
impl From<LocalsError> for VmError {
    fn from(e: LocalsError) -> Self {
        Self::Locals(e)
    }
}
...
impl Vm {
    ...
    fn call_library(
        ...
        // Special case: EVAL
        if lib_id == PROG_LIB && cmd_id == prog_cmd::EVAL {
    ...
    fn dispatch(
        ...
        match op {
            ...
            SymbolicConst => {
                // Deserialize pre-parsed symbolic expression from rodata
                let bytes = self.read_bytes_from_rodata(code, rodata)?;
                let (expr, _) = SymExpr::deserialize(bytes)
                ...
                let program_data = if span_count > 0 {
                    ...
                    for _ in 0..span_count {
                        ...
                        let span_end =
                            read_leb128_u32(code, &mut self.pc).ok_or(VmError::UnexpectedEnd)?;
                        offsets.push(offset);
                        spans.push(Span::new(Pos::new(span_start), Pos::new(span_end)));
                    }
...
mod tests {
    use super::*;
    use crate::bytecode::write_leb128_i64;

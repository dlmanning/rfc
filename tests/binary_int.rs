//! End-to-end tests for binary integer operations.
//!
//! These tests verify the complete source → compile → execute path
//! for binary integer literals and operations (BAND, BOR, etc.).
...
#[test]
fn bnot_64bit() {
    // With 64-bit word size (default), NOT of 0 is all 1s
...
#[test]
fn rcws_default() {
    assert_stack_eq("RCWS", &[64]);
}

#[test]
fn stws_rcws() {
    assert_stack_eq("#10d STWS RCWS", &[10]); // #10d = decimal 10
    assert_stack_eq("#8d STWS RCWS", &[8]); // #8d = decimal 8
    assert_stack_eq("#20d STWS RCWS", &[20]); // #20d = decimal 20
}

#[test]
fn stws_affects_bnot() {
    // With 8-bit word size, NOT of 0xFF is 0
    assert_stack_eq("#8d STWS #FF BNOT", &[0]);
}

#[test]
fn stws_affects_overflow() {
    // With 8-bit word size, 0xFF + 1 wraps to 0
    assert_stack_eq("#8d STWS #FF #1 BADD", &[0]);
}

#[test]
fn stws_invalid_zero() {
    assert_error("#0 STWS");
}

#[test]
fn stws_invalid_65() {
    assert_error("#65d STWS"); // #65d = 65 decimal, which is > 64
}
...
#[test]
fn clear_bit() {
    // Clear bit 4 in #FF: #FF & ~(1 << 4) = #FF & ~#10 = #FF & #...EF
    // With 8-bit word size: #FF & #EF = #EF
    assert_stack_eq("#8d STWS #FF #1 #4 BLSL BNOT BAND", &[0xEF]);
}
...
fn rotate_left_8bit() {
    ...
    assert_stack_eq(
        "#8d STWS #81h DUP #2 BLSL SWAP #6 BLSR BOR #FF BAND",
        &[0x06],
    );
}

#[test]
fn count_with_overflow() {
    // 8-bit counter wrapping: start at #FE, add 3 = #101 & #FF = #01
    assert_stack_eq("#8d STWS #FE #3 BADD", &[0x01]);
}
...
fn swap_nibbles_8bit() {
    ...
    // ((x << 4) | (x >> 4)) & 0xFF
    assert_stack_eq(
        "#8d STWS #AB DUP #4 BLSL SWAP #4 BLSR BOR #FF BAND",
        &[0xBA],
    );
}
...
#[test]
fn arithmetic_in_word_size() {
    // 16-bit arithmetic: #FFFF + 1 wraps to 0
    assert_stack_eq("#10d STWS #FFFF #1 BADD", &[0]);
}
